...
Now, lets open up a terminal in our Docker Container and set our environmental variables.
...
With our variables configured, we can now run Spark Submit on our local cluster.
...
After running `spark-submit` while passing in our script parameters, we can access the job details at 
[http://localhost:4040](http://localhost:4040) during the job runtime, but note it only becomes active and accessible when a Spark application is running. When the job is complete, the UI will no longer be accessible. If you are running some of the scripts in module 5 within a Jupyter Notebook, this will likely not be an issue.
...
Finally, after creating and configuring a [Google Dataproc cluster](https://console.cloud.google.com/dataproc/clusters?project=dtc-de-zoomcamp-410523), we can now use the Google Cloud SDK CLI to run our script on a cluster. Be sure to upload `06_spark_sql.py` to a directory in your bucket, i.e. `your-bucket/code/`.
