...
At the end of this section, I've provided an example snapshot of what a Mage pipeline with dynamic blocks looks like during execution. For the sake of simplicity, I only loaded 3 months of Taxi data. The pipeline works as follows, the `fetch_taxi_url_metadata_csv` block is configured as a "dynamic block". It iterates through a range of 12, represent the total months in a year, then appends each url data to a list of dictionaries that is referenced in our return statement as [download_info] to be passed unto our child block `fetch_taxi_url_data_csv`. 
